...
import com.zhangchao.bean.Employee;
import com.zhangchao.mapper.EmployeeMapper;
...
public class EmployeeService {
     ...
     *      4）、cacheManager：指定缓存管理器；或者cacheResolver指定获取解析器 （二选一）
     *      5）、condition：指定符合条件的情况下才缓存：
     *           condition = "#id>0"
     *           condition = "#a0>1"：第一个参数的值》1的时候才进行缓存。#a0:第一个参数的值
    ...
    */
     ...
     *  
     *  @Cacheable 添加缓存时，默认的key是参数的值，所以更新缓存后，如果
     *  不指定key,会生成新的缓存，导致原先的缓存未更新。
     *	
     *	key = "#result.id" 和 key = "#employee.id" 等价，
     *  result -- > 返回值。
     */
     ...
     * @CacheEvict：缓存清除
     *  key：指定要清除的数据
     *  allEntries = true：指定清除 value="emp"  这个缓存中所有的数据
